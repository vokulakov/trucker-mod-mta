...
function dbUpdateHouse(result, params)
    ...
    if (success) then
        ...
        if (houseData and createdHouses[houseId]) then
            ...
            triggerClientEvent(root, 'tmtaHouse.onHouseUpdate', createdHouses[houseId].houseMarker, houseData)
        end
    end
...
function House.create(houseData)
    ...
    if houseData.userId then
        ...
        houseData.owner = result.nickname
    end
...
function House.buy(player, houseId)
    ...
    if (houseData.userId) then
        ...
        return triggerClientEvent(player, 'tmtaHouse.showNotice', resourceRoot, 'error', 'Ошибка покупки дома. Обратитесь к Администратору!')
    end
    ...
    if (exports.tmtaRevenueService:isPlayerHasPropertyTaxDebt(player)) then
        return triggerClientEvent(player, 'tmtaHouse.showNotice', resourceRoot, 'error', 'Вы не можете купить дом, имея задолженности по налогу на недвижимость!')
    end
...
function dbBuyHouse(result, params)
    ...
    if success then
        ...
        exports.tmtaGarage:addPlayerGarageSlot(player, houseData[1].parkingSpaces)
    end
...
function dbSellHouse(result, params)
    ...
    if success then
        ...
        if (isElement(player)) then
            ...
            exports.tmtaGarage:removePlayerGarageSlot(player, parkingSpaces)
            ...
            exports.tmtaCore:giveUserMoney(userId, price)
            exports.tmtaGarage:removeUserGarageSlot(userId, parkingSpaces)
        end
