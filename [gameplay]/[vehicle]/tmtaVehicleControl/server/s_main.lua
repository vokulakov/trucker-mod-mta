...
addEvent('onVehicleChangeEngineState', true)
...
function doToggleEngine(player)
	...
	triggerEvent('onVehicleChangeEngineState', veh, getVehicleEngineState(veh))
end
...
local function setVehicleLightOn(veh, rl, gl, bl)
	...
	local colorLight = getElementData(veh, 'HeadLightColor') or {255, 255, 255}
...
function doToggleLights(player)
	...
	if light == 0 then
		...
		local colorLight = getElementData(veh, 'HeadLightColor')
		if colorLight then
		...
		setElementData(veh, 'HeadLightColor', {rl, gl, bl})
...
addEventHandler('onElementDataChange', root, 
	function(dataName, _, value)
		...
		if (isElement(vehicle) and getElementType(vehicle) == 'vehicle' and dataName == "vehicle.vehLightState") then
			local trailer = getVehicleTowedByVehicle(vehicle)
			if (isElement(trailer) and trailer:getData('isTrailer')) then
				local value = value > 0 and 2 or 1
				setVehicleOverrideLights(trailer, value)
			end
		end
	end
)
...
addEventHandler('onTrailerAttach', root, 
    function(truck)
		...
		if (isElement(trailer) and trailer:getData('isTrailer') and getVehicleTowedByVehicle(truck) == trailer) then
			setVehicleOverrideLights(trailer, getVehicleOverrideLights(truck))
		end
    end
)
