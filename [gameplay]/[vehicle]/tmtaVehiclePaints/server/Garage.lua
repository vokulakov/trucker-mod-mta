...
function Garage.create(id, position, size)
    ...
    --createBlipAttachedTo(cuboid, Config.garageMarkerBlip)
    GaragesList[cuboid] = {}
...
function Garage.playerEnter(player)
    ...
    triggerClientEvent(player, "tmtaVehPaint.onPlayerEnterGarage", player)
end

addEvent("tmtaVehPaint.onPlayerEnterGarage", true)
addEventHandler("tmtaVehPaint.onPlayerEnterGarage", root, function()
    local player = client
...
function Garage.playerExit(player)
    ...
    if not PlayersInPaint[player] then
        return
    end
    ...
    player.interior = 0
    PlayersInPaint[player] = nil
    Garage.clearPlayerDimension(player)
	
    triggerClientEvent(player, "tmtaVehPaint.onPlayerExitGarage", player)

    fadeCamera(player, true, 1)
end

addEvent("tmtaVehPaint.onPlayerExitGarage", true)
addEventHandler("tmtaVehPaint.onPlayerExitGarage", root, function()
