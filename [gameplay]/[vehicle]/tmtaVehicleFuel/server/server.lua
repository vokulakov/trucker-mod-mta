...
addEventHandler("onResourceStart", resourceRoot, function()
	...
	for ID, node in ipairs(xmlNodeGetChildren(xmlFuelingData)) do
		...
		local coeff = priceCoeff[math.random(1, #priceCoeff)]
		local price = {}
		for fuelType, fuelPrice in pairs(priceSetting) do
			price[fuelType] = math.floor(fuelPrice*coeff)
		end
		...
		for i, v in ipairs(moreKids) do
			...
			REFILLS[ID][i].pickup:setData('3dText', 'АЗС')

			setElementData(REFILLS[ID][i].pickup, 'exv_fuelSystem.isRefills', true)
			setElementData(REFILLS[ID][i].colSphere, 'exv_fuelSystem.priceSetting', price)
...
function onVehicleColSphereHit(vehicle)
	...
	triggerClientEvent(driver, 'fuel_system.showFuelingWindow', driver, source:getData('exv_fuelSystem.priceSetting'))
end
...
function setVehicleFuel(veh)
	...
	setElementData(veh, 'fuel', getElementData(veh, 'fuel') or vehiclesData[model].fuel)
	setElementData(veh, 'fuelMax', getElementData(veh, 'fuelMax') or vehiclesData[model].fuel)
	setElementData(veh, 'typeOfFuel', getElementData(veh, 'typeOfFuel') or vehiclesData[model].typeOfFuel)
...
function addCarFuel(veh, amount, money)
	if not veh then return end
	setElementData(veh, 'fuel', getElementData(veh, 'fuel') + amount)
...
addEventHandler('onVehicleChangeEngineState', root,
