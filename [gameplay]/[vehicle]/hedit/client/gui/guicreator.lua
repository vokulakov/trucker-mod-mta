...
function buildMainWindow()
    ...
    heditGUI.btn_close = guiCreateButton(wnd.size[1] - 150, 21, 150, 20, "Закрыть", false, heditGUI.window)
    guiSetFont(heditGUI.btn_close, "default-bold-small")
    guiSetProperty(heditGUI.btn_close, "NormalTextColour", "fff01a21")
    heditGUI.btn_close:setData('tmtaSounds.UI', 'ui_back')

    addEventHandler("onClientGUIClick", heditGUI.btn_close, function()
        if not isElement(heditGUI.window) then 
            return
        end 
        setVisible(false)
        exports.tmtaVehTuning:setWindowVisible(true)
    end)
