...
local LOGO_TEXTURE = exports.tmtaTextures:createTexture('logo_tmta_64')
...
local function drawBlackBar()
	dxDrawRectangle(0, 0, screenW, 40, tocolor(0, 0, 0, 255), true)

	local posY = screenH-40
	dxDrawRectangle(0, posY, screenW, 40, tocolor(0, 0, 0, 255), true)

	local posX = screenW-64
	posY = posY-10-64

	dxSetTextureEdge(LOGO_TEXTURE, 'clamp')
	dxDrawImage(screenW*((posX) /sDW), screenH*((posY) /sDH), screenW*((64) /sDW), screenH*((64) /sDH), LOGO_TEXTURE, 0, 0, 0, tocolor(255, 255, 255, 155), false)

	dxDrawText("TRUCKER × MTA", screenW*((0) /sDW), screenH*((posY-10) /sDH), screenW*((posX) /sDW), screenH*((posY+64) /sDH), tocolor(255, 255, 255, 255), screenW/sDW*1.0, FONTS.RR_14, 'right', 'center')
	dxDrawText("truckermta.ru", screenW*((0) /sDW), screenH*((posY+20) /sDH), screenW*((posX) /sDW), screenH*((posY+64) /sDH), tocolor(255, 255, 255, 155), screenW/sDW*1.0, FONTS.RR_10, 'right', 'center')
end
...
function startCamHack()
	...
	addEventHandler("onClientPreRender", root, drawBlackBar)
...
function stopCamHack(  )
	...
	removeEventHandler("onClientPreRender", root, drawBlackBar)
