...
function resStatRender()
	...
	for i, row in ipairs(rows) do
		local _color = tocolor(255,255,255,255)
		local text = row[1]:sub(0,15)..": "..row[2]

		local statTiming = string.gsub(row[2], '%%', '')
		statTiming = tonumber(statTiming)
		if (statTiming and statTiming >= 1.8) then
			if statTiming >= 15 then
				_color = tocolor(255, 0, 0, 255)
			elseif statTiming >= 6 then
				_color = tocolor(255, 255, 0, 255)
			end
			dxDrawText(text,x,y,150,15,_color, 1, "default-bold")
			y = y + 15
		end
	end
