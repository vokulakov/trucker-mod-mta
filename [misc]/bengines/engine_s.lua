...
function addVehicleEngine(vehicle)
	...
	local vehicleType = getElementData(vehicle, "vehicle:type")
	local model = getElementModel(vehicle)
	
	if data and ENGINE_DATA[vehicleType] and ENGINE_DATA[vehicleType][data] then 
		local upgrades = getElementData(vehicle, "vehicle:upgrades") or {engine={turbo=false}}
		engineData = ENGINE_DATA[vehicleType][data]
		engineData.name = data 
		engineData.volMult = SOUNDPACK_VOLUME[engineData.soundPack] or 1
		engineData.turbo = upgrades.engine and upgrades.engine.turbo or false
		engineData.turbo_shifts = engineData.turbo
		
		setElementData(vehicle, "vehicle:engine", engineData)
		setElementData(vehicle, "vehicle:fuel_type", engineData.fuel)
		...
		outputDebugString("Engine data not found for the vehicle type and data.")
	end
end 
