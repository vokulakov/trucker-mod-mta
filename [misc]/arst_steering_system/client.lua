...
function()
	for vehicle, v in pairs (rotatingVehicles) do
		if isElement(vehicle) then
			if v.rotating == 1 then
				rotatingVehicles[vehicle].wheel_rotation = interpolateBetween ( v.wheel_rotation, 0, 0, 90, 0, 0, (getTickCount()-v.rotate_start)/wheel_rotate_time, "Linear" )
			elseif v.rotating == 2 then
				rotatingVehicles[vehicle].wheel_rotation = interpolateBetween ( v.wheel_rotation, 0, 0, 0, 0, 0, (getTickCount()-v.rotate_start)/wheel_rotate_time, "Linear" )
			elseif v.rotating == 3 then
				rotatingVehicles[vehicle].wheel_rotation = interpolateBetween ( v.wheel_rotation, 0, 0, -90, 0, 0, (getTickCount()-v.rotate_start)/wheel_rotate_time, "Linear" )
			end
		--	outputChatBox(rotatingVehicles[i].wheel_rotation)
			setVehicleComponentRotation ( vehicle, v.name, 0, rotatingVehicles[vehicle].wheel_rotation, 0 )
		end
	end
end)
