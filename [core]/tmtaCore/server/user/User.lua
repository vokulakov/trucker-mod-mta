...
function User.setup()
    exports.tmtaSQLite:dbTableCreate(USER_TABLE_NAME, {
        ...
        -- Количество слотов для хранения номерных знаков
        {name = "licensePlateSlot", type = "INTEGER", options = "DEFAULT 0 NOT NULL"},
    ...
    --exports.tmtaSQLite:dbTableAddColumn(USER_TABLE_NAME, 'licensePlateSlot', 'INTEGER')
...
function callbackLoginPlayer(result, params)
    ...
    if (success) then
		if (Session.start(player)) then
            ...
            exports.tmtaVehicle:updatePlayerVehiclesCount(player)
    ...
    triggerClientEvent(player, resourceName..".onClientPlayerLogin", resourceRoot, success, errorType)
end
...
function User.getById(userId, fields, callbackFunctionName, ...)
    ...
    local result = exports.tmtaSQLite:dbTableSelect(USER_TABLE_NAME, fields, {userId = userId}, callbackFunctionName, ...)
    if not result then
        return false
    end
