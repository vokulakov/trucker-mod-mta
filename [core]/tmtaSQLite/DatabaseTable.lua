...
function DatabaseTable.prepareQueryString(queryString, ...)
	if type(queryString) ~= "string" then
		outputDebugString("ERROR: DatabaseTable.prepareQueryString: bad arguments")
		return false
	end

	local connection = Database.getConnection()
	if not connection then
		outputDebugString("ERROR: DatabaseTable.prepareQueryString: no database connection")
		return false
	end

	return connection:prepareString(queryString, ...)
end
...
dbPrepareQueryString = DatabaseTable.prepareQueryString