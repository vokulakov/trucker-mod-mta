...
function Confirm.setTitle(confirm, title)
    if (not createdConfirm[confirm] or type(title) ~= 'string' or title:len() == 0) then
        return false
    end
    createdConfirm[confirm].title.text = title
end
...
function Confirm.setBtnCancelLabel(confirm, label)
    if (not createdConfirm[confirm] or type(label) ~= 'string' or label:len() == 0) then
        return false
    end
    createdConfirm[confirm].btnCancel.text = label
end

-- TODO:
function Confirm.setSise(confirm, width, height)
end

-- Exports
createConfirm = Confirm.create
deleteConfirm = Confirm.delete
confirmSetBtnOkLabel = Confirm.setBtnOkLabel
confirmSetBtnCancelLabel = Confirm.setBtnCancelLabel
confirmSetTitle = Confirm.setTitle