...
local function getFormatedMileageString(mileage)
    local unitKm, unitM = exports.tmtaUtils:convertMetersToKm(mileage)
...
function Speed.draw()
    ...
    -- FUEL
    local vehicleFuel = veh:getData('fuel') or 0
	local vehicleFuelMax = veh:getData('fuelMax') or 0
    ...
    -- NOS
    if not veh:getData('nitroBlocked') then
        ...
        dxDrawText('0%', sW*((posX) /sDW), sH*((posY) /sDH), sW*((posX+width) /sDW), sH*((posY+height-15) /sDH), nosColor, sW/sDW*1.0, Fonts.GPB_7, "right", "bottom", false, false, false, true)
    end
    ...
    if currentMileage then
        dxDrawText(getFormatedMileageString(currentMileage), sW*((posX) /sDW), sH*((posY) /sDH), sW*((posX+width) /sDW), sH*((posY+height-27.5+5) /sDH), tocolor(255, 255, 255, 255), sW/sDW*1.0, Fonts.GPM_9, "center", "bottom", false, false, false, true)
    end
...
local function vehicleTurnlightAnim(value)
    setAnimData('turn-alpha', 0.08, value)
    local _value = (value == 1) and 0 or 1
    animate('turn-alpha', _value, function()
        setTimer(vehicleTurnlightAnim, 200, 1, _value)
    end)
end
