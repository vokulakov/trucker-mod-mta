...
local function drawBlips()
    for _, blip in ipairs(getElementsByType("blip")) do
        ...
        if (blipIcon and Textures[blipIcon]) then
            ...
            if ((distance <= blip.visibleDistance and distance <= MAX_DISTANCE_BLIP_VISIBLE) or exceptionsBlip[blipIcon]) then
                ...
                local blipColor = blip:getData("color") or tocolor(255, 255, 255, 255)
                local isVisible = true
    
                if (blipX == 0 or blipX == radarWidth or blipY == 0 or blipY == radarHeight) then
                    local r, g, b, a = exports.tmtaUtils:fromColor(blipColor)
