...
function HUD.draw()
    ...
    dxDrawImage(sW*((posX) /sDW), sH*((posY+5) /sDH), sW*((80) /sDW), sH*((80) /sDH), Textures.logo, 15, 0, 0, tocolor(255, 255, 255, 255), false)
    
    -- X2
    if exports.tmtaX2:isPromoActive() then
        local offsetPosY = 76
        dxSetTextureEdge(Textures.bg_x2, 'clamp')
        dxDrawImage(sW*((sW-45) /sDW), sH*((posY+offsetPosY) /sDH), sW*((50) /sDW), sH*((50) /sDH), Textures.bg_x2, 0, 0, 0, tocolor(255, 255, 255, 255), false)
        dxDrawText("x2", sW*((sW-45) /sDW), sH*((posY+offsetPosY) /sDH), sW*((sW) /sDW), sH*((posY+offsetPosY+50) /sDH), tocolor(255, 255, 255, 255), 1, Fonts.X2, 'center', 'center')
    end
