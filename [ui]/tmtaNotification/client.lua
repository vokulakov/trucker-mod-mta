...
addEventHandler("onClientRender", getRootElement(),
	function ()
		...
		if not exports.tmtaUI:isPlayerComponentVisible("notifications") then
			return
		end
		...
		if progress <= 1 then
			--dxDrawRectangle(screenX/2-(200/2)*px, screenY/2+(990/2)*py, 200*px, 2*py, tocolor(255, 255, 255, alpha * 100))
			--dxDrawRectangle(screenX/2-(200/2)*px, screenY/2+(990/2)*py, 200*px*(1 - progress), 2*py, tocolor(240, 146, 115, alpha * 255))
		end
		...
		if infobox.message[2] then
			...
			dxDrawText(infobox.message[2], x, screenY/2+(2750/2)*py, x + infobox.tileWidth, y + infobox.tileHeight, tocolor(255, 255, 255, alpha * 210), 1, fonts[5], "center", "center", true, false, false, true)
		else
